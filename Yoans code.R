# Beta diversity in grassland habitats
# By: Yoan Fourcade
...
### import data ###
site_data <- read.csv2(here("data", "database.PD.taxalabels.csv"))

data_BB <- read_csv2(here("data", "bb.tidy.csv"))
#data_BB$Transect_type[data_BB$Transect_type == "Between fields"] <- "Field border"
data_But <- read_csv2(here("data", "bf.tidy.csv"))
#data_But$Transect_type[data_But$Transect_type == "Between fields"] <- "Field border"
data_Plants <- read_csv2(here("data", "pp.tidy.csv")) %>% 
  group_by(Landscape, Transect_type, Species) %>%
  summarise(Indiv = length(Plot), Category = unique(Category))
#data_Plants$Transect_type[data_Plants$Transect_type == "Between fields"] <- "Field border"
...
for(i in unique(data$taxon)){
  ...
  nmds <- metaMDS(data.nmds[,-c(1:3, (ncol(data.nmds)-19):ncol(data.nmds))], trymax = 1000, maxit = 20000, k=3)
...
p.hab <- ggplot() + 
  geom_polygon(data=ndms.plots.hab[ndms.plots.hab$what == "hull.habitat",],
               aes(x=NMDS1,y=NMDS2,fill=habitat ,group=habitat ),alpha=0.15) + # add the convex hulls
  ...
  geom_point(data=ndms.plots.hab[ndms.plots.hab$what == "scores",],
             aes(x=NMDS1,y=NMDS2,shape=habitat,colour=habitat),size=2) + # add the point markers
  # geom_text(data=ndms.plots.sp,
  #            aes(x=NMDS1,y=NMDS2, label = Species),size=1) + # add the point markers
  facet_grid(~ taxon) +
  coord_equal() +
  scale_color_manual("Habitat type", values = c("#d7191c", "#fdae61", "#e6f598", "#abd9e9", "#2c7bb6"), 
                       labels = c("Field border", "Power line", "Big road", "Small road", "Pasture")) +
  scale_fill_manual("Habitat type", values = c("#d7191c", "#fdae61", "#e6f598", "#abd9e9", "#2c7bb6"), 
                    labels = c("Field border", "Power line", "Big road", "Small road", "Pasture")) +
  scale_shape_discrete("Habitat type", labels = c("Field border", "Power line", "Big road", "Small road", "Pasture")) +
  theme_minimal() +
  theme(strip.text.x = element_blank(), text = element_text(size=16))
...
ggdraw(p.hab) + draw_image(humla, x = 0.23, y = 0.87, hjust = 1, vjust = 1, width = 0.09, height = 0.09) +
  draw_image(maniola, x = 0.499, y = 0.87, hjust = 1, vjust = 1, width = 0.09, height = 0.09) + 
  draw_image(plant, x = 0.77, y = 0.879, hjust = 1, vjust = 1, width = 0.1, height = 0.1) + 
...
p.env <- ggplot() +
  ...
  # geom_text(data=species.scores,aes(x=NMDS1,y=NMDS2,label=species),alpha=0.5, size = 3.5) +  # add the species labels
  geom_point(data=ndms.plots.hab[ndms.plots.hab$what == "scores",],
             aes(x=NMDS1,y=NMDS2,colour=TUVA),size=1.3) + # add the point markers
  facet_grid(~ taxon) +
  scale_colour_gradient("Proportion of\nsemi-natural \npastures", low = c("#ffffb2"), high = c("#bd0026")) +
  scale_fill_manual("", values = c("#0868ac", "#78c679")) +
  coord_equal() +
  theme_minimal()+
...
ggdraw(p.env) + draw_image(humla, x = 0.23, y = 0.87, hjust = 1, vjust = 1, width = 0.09, height = 0.09) +
  draw_image(maniola, x = 0.499, y = 0.87, hjust = 1, vjust = 1, width = 0.09, height = 0.09) + 
  draw_image(plant, x = 0.77, y = 0.879, hjust = 1, vjust = 1, width = 0.1, height = 0.1) + 
  scale_fill_discrete(name = "Habitat type", labels = c("Field border", "Power line", "Big road", "Small road", "Pasture"))
...
# change the labels of the dendrogram
...
# For fig:
tot.beta <-beta.tot %>% filter(Type != "Total") %>% ggplot(aes(x = taxon, y = beta, fill = Type)) + 
  geom_bar(stat = 'identity') +
  scale_y_continuous("Total beta-diversity\nbetween habitat types") +
  scale_x_discrete("") +
  scale_fill_manual("", values = c("#c7eae5", "#01665e")) +
...
# Figure 4 ----
Figure.4 <- ggarrange(tot.beta, humla, maniola, plant, ncol=3, nrow=2, heights = c(12,1), widths = c(9,0.1,0.1))
Figure.4

ggdraw(Figure.4) + draw_image(humla, x = 0.3, y = 0.15, hjust = 1, vjust = 1, width = 0.12, height = 0.12) +
  draw_image(maniola, x = 0.49, y = 0.159, hjust = 1, vjust = 1, width = 0.12, height = 0.12) + 
  draw_image(plant, x = 0.7, y = 0.17, hjust = 1, vjust = 1, width = 0.15, height = 0.15) 
...
pairwise <- beta.pair %>% filter(!rows == vars) %>% 
  ggplot(aes(x=rows, y=vars, fill=beta, label = round(beta,2))) +
  facet_grid(taxon ~ Type) +
  geom_tile() +
  geom_text(size = 4, col = "white") +
  scale_x_discrete(name = "Habitat type", labels = c("Field border", "Big road", "Pasture", "Power line", "Small road")) +
  scale_y_discrete(name = "Habitat type", labels = c("Field border", "Big road", "Pasture", "Power line", "Small road")) +
...
# Figure S2 ----
ggdraw(pairwise) + draw_image(humla, x = 1, y = 0.91, hjust = 1, vjust = 1, width = 0.12, height = 0.12) +
  draw_image(maniola, x = 1, y = 0.62, hjust = 1, vjust = 1, width = 0.12, height = 0.12) + 
  draw_image(plant, x = 1, y = 0.38, hjust = 1, vjust = 1, width = 0.15, height = 0.15) 
