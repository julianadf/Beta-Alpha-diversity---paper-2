...
bf.BR <- bf.scores[bf.scores$habitat == "Big road", ][chull(bf.scores[bf.scores$habitat =="Big road", c("NMDS1", "NMDS2")]), ]  # hull 
bf.SR <- bf.scores[bf.scores$habitat == "Small road", ][chull(bf.scores[bf.scores$habitat =="Small road", c("NMDS1", "NMDS2")]), ]  # hull values 

hull.bf <- rbind(bf.pasture, bf.BR, bf.SR)  #combine landscapes
hull.bf
...
gg.bf<- ggplot() + 
  ...
  scale_color_manual("habitat", values = c("#969696",  "#3288bd", "#d53e4f", "#abd9e9", "#d7191c")) +
  scale_fill_manual("habitat", values = c("#969696",  "#3288bd", "#d53e4f", "#abd9e9", "#d7191c")) +
...
ggdraw(gg.bf) + draw_image(maniola, x = 0.97, y = 0.99, hjust = 1, vjust = 1, width = 0.15, height = 0.15) +
  draw_image(a, x = 0.18, y = 1, hjust = 1, vjust = 1, width = 0.1, height = 0.1)
...
bb.BR <- bb.scores[bb.scores$habitat == "Big road", ][chull(bb.scores[bb.scores$habitat =="Big road", c("NMDS1", "NMDS2")]), ]  # hull 
bb.SR <- bb.scores[bb.scores$habitat == "Small road", ][chull(bb.scores[bb.scores$habitat =="Small road", c("NMDS1", "NMDS2")]), ]  # hull values 

hull.bb <- rbind(bb.pasture, bb.BR, bb.SR)  #combine landscapes
hull.bb
...
ggdraw(gg.bb) + draw_image(terrestris, x = 0.82, y = 0.98, hjust = 1, vjust = 1, width = 0.12, height = 0.12) +
  draw_image(b, x = 0.35, y = 1, hjust = 1, vjust = 1, width = 0.1, height = 0.1) 
...
pp.BR <- pp.scores[pp.scores$habitat == "Big road", ][chull(pp.scores[pp.scores$habitat =="Big road", c("NMDS1", "NMDS2")]), ]  # hull 
pp.SR <- pp.scores[pp.scores$habitat == "Small road", ][chull(pp.scores[pp.scores$habitat =="Small road", c("NMDS1", "NMDS2")]), ]  # hull values 

hull.pp <- rbind(pp.pasture, pp.BR, pp.SR)  #combine landscapes
hull.pp
...
ggdraw(gg.pp) + draw_image(fragaria, x = 0.85, y = 0.98, hjust = 1, vjust = 1, width = 0.22, height = 0.22) +
  draw_image(c, x = 0.35, y = 1, hjust = 1, vjust = 1, width = 0.1, height = 0.1)
