...
scorez <- as.data.frame(scores(nmds, "species"))
scorez$species <- rownames(scorez)  # create a column of species, from the rownames of species.scores
head(scorez)  
...
p.hab <- ggplot() + 
  ...
  geom_text(data=scorez,aes(x=NMDS1,y=NMDS2,label=species),alpha=0.5, size = 3.5) +  # add the species labels
...
pairwise_permanova %>% p.adjust() %>% kable(format = "pipe", digits = 3)
